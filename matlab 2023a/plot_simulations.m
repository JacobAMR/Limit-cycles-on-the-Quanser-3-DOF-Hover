function [] = plot_simulations(xYaw, yYaw, xPitch, yPitch, xRoll, yRoll, voltF, voltB, voltR, voltL, lStart, initials, tStart, tTime, fontSize, savePlot, plotName, simuName, cycleType, uM)
    close all
    vLength = length(xYaw);

    %y = (eq1 - u10)^2 + (eq2 - u20)^2 + 0.1*min(eq0, 0);
    ...
    xlabel('Angle')
    ylabel('Velocity')
    %Lgnd = legend(arrayfun(@(mode) sprintf('x0%d', mode), 1:size(yYaw, 2), 'UniformOutput', false));
    Lgnd = legend(arrayfun(@(mode) sprintf('test %d', mode), 1:size(yYaw, 2), 'UniformOutput', false));
    ...
    ylabel('Pitch')
    zlabel('Roll')
    %legend(arrayfun(@(mode) sprintf('x0%d', mode), 1:size(yYaw, 2), 'UniformOutput', false))
    ...
    if savePlot == 1
        ...
        savefig(fig3, append('images/_pitch-roll_fig_', simuName))
        %saveas(fig3, append('images/', plotName, '_pitch-roll_', simuName, '.png'))
    end
    ...
    xticks([])
    ylabel('Voltage (V)')
    %Lgnd = legend(arrayfun(@(mode) sprintf('x0%d', mode), 1:size(voltF, 2), 'UniformOutput', false));
    Lgnd = legend(arrayfun(@(mode) sprintf('test %d', mode), 1:size(yYaw, 2), 'UniformOutput', false));
    %Lgnd = legend(legendText)
    ...
    hold on
    for n = 1:initials
        plot(timeVector, x1Vector(tStart:end, n), 'LineStyle', ':')
    end
    title('Pitch angle')
    %xticks([])
    ...
    end_index = 1;
    transformables = [x1Vector, x2Vector];
    ...
    for n = 1:initials
        ...
        assignin('base','fPitch', f);
        assignin('base','FrPitch', Fr);
        plot(f, Fr, 'LineStyle', ':')
    end
    title('Pitch fourier')
    %xticks([])
    ...
    hold on
    for n = 1:initials
        plot(timeVector, x2Vector(tStart:end, n), 'LineStyle', ':')
    end
    title('Roll angle')
    %xticks([])
    ylabel('Angle (degrees)')
    ...
    hold on    
    for n = 1:initials
        fourier = fft(x2Vector(:, n));
        fourier_half = fourier(1:N/2);
        f = fs*(0:N/2-1)/N;
        Fr = abs(fourier_half)/(N/2);
        %%%%%% But the question is, can I select the sampling frequency
        %%%%%% freely or is that something I should be finding out?
        assignin('base','fRoll', f);
        assignin('base','FrRoll', Fr);
        plot(f, Fr, 'LineStyle', ':')
    end
    ...
    if savePlot == 1
        saveas(fig8, append('images/', plotName, '_fourier_', simuName, '.png'))
    end
end