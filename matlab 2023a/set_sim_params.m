function [plotName, startTime, stopTime, space, x0M] = set_sim_params(long, sampleTime, plotName, plotMax, simuName, sat, method, der, sets, initials, cycle, feedbackType, cycleType, paramTest)
    ...
    elseif sets == 4
        x0M(1:6, 1) = transpose(pi/180*[8, 8, 8, 0, 0, 0]);
    ...
    elseif sets == 6
        x0M(1:6, 1) = transpose(2*pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 2) = transpose(pi/180*[5, 5, 5, 0, 0, 0]);
        x0M(1:6, 3) = transpose(-pi/180*[2, 2, 2, 0, 0, 0]);
    elseif sets == 7
        x0M(1:6, 1) = transpose(8*pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 2) = transpose(pi/180*[2, 2, 2, 0, 0, 0]);
        x0M(1:6, 3) = transpose(-pi/180*[1, 1, 1, 0, 0, 0]);
    elseif sets == 8
        x0M(1:6, 1) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 2) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 3) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 4) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 5) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 6) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 7) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 8) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 9) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
        x0M(1:6, 10) = transpose(pi/180*[1, 1, 1, 0, 0, 0]);
    elseif sets == 9
        x0M(1:6, 1) = transpose(pi/180*[0.0001, 0.0001, 0.0001, 0, 0, 0]);
    end
